 ...
 * ✅ CORRIGIDO: Removido imports ES6, usa window globals
 * Data: 10/10/2025
 */

// ✅ Aguarda window globals estarem disponíveis
function waitForDependencies(callback, maxAttempts = 100, attempt = 0) {
  if (window.AlshamSupabase?.supabase && window.showNotification) {
    console.log('✅ Dependências carregadas para Pipeline');
    callback();
  } else if (attempt >= maxAttempts) {
    console.error('❌ Dependências não carregaram após 10 segundos');
  } else {
...
waitForDependencies(() => {
  // ✅ Usar window globals ao invés de imports
  ...
  window.toggleGlobalSound = function () {
    ...
    updateSoundButtonUI();
    if (window.notify && typeof window.notify.setSoundPreference === 'function') {
      window.notify.setSoundPreference(soundEnabled);
    }
  ...
  console.log('🚀 Pipeline v6.1 carregado (window globals)');
});
