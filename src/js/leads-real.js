/**
 * ALSHAM 360° PRIMA - LEADS REAIS V6.1 FINAL
 ...
 * ✅ COMPLETO: Todas as funções de renderização incluídas
 */
...
function showLoading(show, msg = "Carregando...") {
  ...
  if (show) {
    // Remove loading indicator inicial se existir
    if (loadingIndicator) {
...
waitForSupabase(() => {
  ...
  async function loadSystemData() {
    ...
    try {
      ...
      console.log("✅ Dados carregados:", { leads: leadsState.leads.length, kpis: leadsState.kpis });
    } finally {
    ...
    container.innerHTML = `
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white p-4 rounded-lg shadow hover:shadow-md transition-shadow">
          ...
          <h2 class="text-3xl font-bold text-purple-600">${kpis.conversao || 0}%</h2>
        </div>
        <div class="bg-white p-4 rounded-lg shadow hover:shadow-md transition-shadow">
          <p class="text-gray-600 text-sm mb-1">Pontos Gamificação</p>
    ...
    container.innerHTML = `
      <div class="flex flex-wrap gap-3">
        <input
          ...
          class="flex-1 min-w-[200px] border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        >
        <select id="filter-status" class="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500">
  ...
  function renderTable() {
    ...
    if (rows.length === 0) {
      container.innerHTML = `
        <div class="text-center py-12">
          ...
          <p class="text-gray-600 text-lg font-medium">Nenhum lead encontrado</p>
          <p class="text-gray-500 text-sm mt-2">Ajuste os filtros ou adicione um novo lead</p>
        </div>
      `;
      return;
    }
        ...
        <table class="w-full border-collapse min-w-[900px]">
          <thead>
            <tr class="bg-gray-100 border-b-2 border-gray-300">
              <th class="p-3 text-left font-semibold text-sm">Nome</th>
              <th class="p-3 text-left font-semibold text-sm">Email</th>
              <th class="p-3 text-left font-semibold text-sm">Telefone</th>
              <th class="p-3 text-left font-semibold text-sm">Empresa</th>
              <th class="p-3 text-left font-semibold text-sm">Status</th>
              <th class="p-3 text-left font-semibold text-sm">Origem</th>
              <th class="p-3 text-left font-semibold text-sm">Score IA</th>
              <th class="p-3 text-left font-semibold text-sm">Data</th>
            </tr>
          </thead>
          <tbody>
            ${rows.map(l => {
    ...
    container.querySelectorAll('tr[data-lead-id]').forEach(row => {
      row.addEventListener('click', () => {
        window.openLeadModal(row.getAttribute('data-lead-id'));
      });
    });
  }
          ...
          <div class="overflow-y-auto p-6">
            <form id="new-lead-form" class="space-y-4">
              <div>
                ...
                <input type="text" id="new-lead-nome" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Ex: João Silva">
              ...
              <div>
                ...
                <input type="email" id="new-lead-email" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="exemplo@empresa.com.br">
              </div>
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Telefone</label>
                <input type="tel" id="new-lead-telefone" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="(11) 99999-9999" maxlength="15">
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-semibold text-gray-700 mb-1">Empresa</label>
                  <input type="text" id="new-lead-empresa" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Nome da empresa">
                </div>
                <div>
                  <label class="block text-sm font-semibold text-gray-700 mb-1">Cargo</label>
                  <input type="text" id="new-lead-cargo" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Ex: Gerente de TI">
                </div>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-semibold text-gray-700 mb-1">Status</label>
                  <select id="new-lead-status" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ${LEADS_CONFIG.statusOptions.map(s => `<option value="${s.value}">${s.icon} ${s.label}</option>`).join('')}
                  </select>
                </div>
                <div>
                  <label class="block text-sm font-semibold text-gray-700 mb-1">Origem</label>
                  <select id="new-lead-origem" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
              ...
              <div>
                ...
                <textarea id="new-lead-observacoes" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" placeholder="Informações adicionais sobre o lead..."></textarea>
              </div>
              <div class="flex gap-3 pt-4 border-t border-gray-200">
                <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-semibold text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Criar Lead</button>
                <button type="button" id="cancel-new-lead" class="px-6 bg-gray-100 hover:bg-gray-200 text-gray-700 py-3 rounded-lg font-semibold text-sm transition-colors">Cancelar</button>
              </div>
            </form>
          </div>
        </div>
  ...
  window.createNewLead = async function() {
    ...
    try {
      ...
      const { data, error } = await genericInsert("leads_crm", leadData);
      if (error) throw error;
  ...
  window.openLeadModal = async function(leadId) {
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="space-y-4">
          <div>
            <div class="flex justify-between items-center mb-1 gap-2">
              ...
              <div class="flex gap-2">
                <button data-edit-lead="${lead.id}" class="px-3 py-1 bg-yellow-500 hover:bg-yellow-600 text-white text-sm rounded transition-colors font-medium flex items-center gap-1">✏️ Editar</button>
                <button data-delete-lead="${lead.id}" class="px-3 py-1 bg-red-500 hover:bg-red-600 text-white text-sm rounded transition-colors font-medium flex items-center gap-1">🗑️ Deletar</button>
              </div>
            </div>
          ...
          <div class="bg-gray-50 rounded-lg p-4 space-y-3">
            ...
            <div class="space-y-2 text-sm">
              <div class="flex items-start"><span class="font-medium text-gray-600 w-24">Email:</span><span class="text-gray-900">${lead.email || "-"}</span></div>
              <div class="flex items-start"><span class="font-medium text-gray-600 w-24">Telefone:</span><span class="text-gray-900">${lead.telefone || "-"}</span></div>
            ...
            <div class="space-y-2">
              <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button data-recalc-lead="${lead.id}" class="px-2 py-1 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded transition-colors">Recalcular</button>
                </div>
              </div>
        ...
        <div class="space-y-4">
          <div class="flex justify-between items-center">
            ...
            <button data-add-interaction="${leadId}" class="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded transition-colors font-medium">+ Nova Interação</button>
          </div>
    ...
    if (recalcBtn) recalcBtn.addEventListener('click', () => window.recalculateLeadScore(recalcBtn.dataset.recalcLead));
    
    const addInteractionBtn = modal.querySelector('[data-add-interaction]');
    if (addInteractionBtn) addInteractionBtn.addEventListener('click', () => window.showAddInteractionForm(addInteractionBtn.dataset.addInteraction));
    
    const editBtn = modal.querySelector('[data-edit-lead]');
    if (editBtn) editBtn.addEventListener('click', () => window.openEditLeadModal(editBtn.dataset.editLead));
    
    const deleteBtn = modal.querySelector('[data-delete-lead]');
    if (deleteBtn) deleteBtn.addEventListener('click', () => window.openDeleteLeadModal(deleteBtn.dataset.deleteLead));
        ...
        <div class="overflow-y-auto p-6">
          <form id="edit-lead-form" class="space-y-4">
            <div>
              ...
              <input type="text" id="edit-lead-nome" required value="${lead.nome || ''}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Email <span class="text-red-500">*</span></label>
              <input type="email" id="edit-lead-email" required value="${lead.email || ''}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Telefone</label>
              <input type="tel" id="edit-lead-telefone" value="${lead.telefone || ''}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" maxlength="15">
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Empresa</label>
                <input type="text" id="edit-lead-empresa" value="${lead.empresa || ''}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
              </div>
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Cargo</label>
                <input type="text" id="edit-lead-cargo" value="${lead.cargo || ''}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
              </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Status</label>
                <select id="edit-lead-status" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                  ${LEADS_CONFIG.statusOptions.map(s => `<option value="${s.value}" ${s.value === lead.status ? 'selected' : ''}>${s.icon} ${s.label}</option>`).join('')}
                </select>
              </div>
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Origem</label>
                <select id="edit-lead-origem" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                  <option value="">Selecione...</option>
            ...
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Observações</label>
              <textarea id="edit-lead-observacoes" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">${lead.observacoes || ''}</textarea>
            </div>
            <div class="flex gap-3 pt-4 border-t border-gray-200">
              <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-semibold text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Salvar Alterações</button>
              <button type="button" id="cancel-edit-lead" class="px-6 bg-gray-100 hover:bg-gray-200 text-gray-700 py-3 rounded-lg font-semibold text-sm transition-colors">Cancelar</button>
            </div>
          </form>
        </div>
      </div>
  ...
  window.updateLead = async function(leadId) {
    ...
    try {
      ...
      const { data, error } = await genericUpdate("leads_crm", { id: leadId }, updateData);
      if (error) throw error;
      ...
      setTimeout(async () => await loadSystemData(), 1000);
    } catch (error) {
      showLoading(false);
  ...
  window.openDeleteLeadModal = function(leadId) {
      ...
      <div class="bg-white rounded-lg shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div class="p-6">
          ...
          <div class="flex gap-3">
            <button id="confirm-delete-lead" class="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-lg font-semibold text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">Confirmar</button>
            <button id="cancel-delete-lead" class="px-6 bg-gray-100 hover:bg-gray-200 text-gray-700 py-3 rounded-lg font-semibold text-sm transition-colors">Cancelar</button>
          </div>
        </div>
      </div>
    `;
    ...
    modal.addEventListener("click", (e) => { if (e.target === modal) modal.remove(); });
    document.getElementById("confirm-delete-lead").addEventListener("click", async () => await window.deleteLead(leadId));
  ...
  window.deleteLead = async function(leadId) {
    try {
      ...
      const { data, error } = await genericDelete("leads_crm", leadId);
      if (error) throw error;
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      console.log("✅ Usuário autenticado:", leadsState.user.email);
      console.log("✅ Org ID:", leadsState.orgId);
