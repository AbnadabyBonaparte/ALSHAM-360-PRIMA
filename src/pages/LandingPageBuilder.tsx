...
const BlockPreview = ({ block, selected, onSelect, device }: { block: Block; selected: boolean; onSelect: () => void; device: Device }) => {
  ...
  return (
    <Reorder.Item value={block} className="relative">
      ...
      >
        {selected && (
          <div className="absolute -top-5 left-1/2 -translate-x-1/2 z-20 bg-gradient-to-r from-purple-600 to-pink-600 text-[var(--text)] text-xs font-black px-6 py-2 rounded-full uppercase tracking-widest shadow-2xl">
        ...
        <div className={`${deviceWidth} mx-auto`}>
          ...
          {block.type === 'hero' && (
            <div className={`py-40 px-12 text-center ${block.content.bg}`}>
              <h1 className="text-xl md:text-2xl lg:text-3xl md:text-4xl md:text-5xl lg:text-6xl font-black text-[var(--text)] mb-10">{block.content.headline}</h1>
          ...
          {/* FEATURES */}
          {block.type === 'features' && (
            <div className="py-40 px-12 bg-[var(--bg)]/60">
              <h2 className="text-xl md:text-2xl lg:text-3xl font-black text-center text-[var(--text)] mb-32">{block.content.title}</h2>
          ...
          {block.type === 'cta' && (
            <div className={`py-48 px-12 text-center ${block.content.bg}`}>
              <h2 className="text-2xl md:text-3xl lg:text-4xl font-black text-[var(--text)] mb-20">{block.content.title}</h2>
              <button className="px-32 py-16 bg-[var(--bg)] text-[var(--text)] text-6xl font-black rounded-3xl border-8 border-white/30 hover:scale-110 transition-transform shadow-2xl">
          ...
          {block.type === 'testimonials' && (
            <div className="py-48 px-12 bg-[var(--bg)]/80 text-center">
          ...
          {block.type === 'pricing' && (
            <div className="py-48 px-12 bg-gradient-to-b from-black to-purple-950/50 text-center">
              ...
              <h2 className="text-2xl md:text-3xl lg:text-4xl font-black text-[var(--text)] mb-8">{block.content.plan}</h2>
...
export default function VortexArchitect() {
  ...
  return (
    <div className="h-screen flex bg-[var(--bg)] text-[var(--text)]">
        {/* BIBLIOTECA DIVINA */}
        <div className="w-96 border-r border-[var(--border)] bg-gradient-to-b from-[var(--bg)] to-purple-950/30 p-12">
        ...
        <div className="flex-1 flex flex-col">
          <div className="h-24 border-b border-white/5 bg-[var(--bg)]/80 flex items-center justify-between px-16">
            ...
            <div className="flex items-center gap-8">
              ...
              <div className="flex gap-4 bg-white/5 rounded-2xl p-2">
                <button onClick={() => setDevice('desktop')} className={`${device === 'desktop' ? 'bg-white text-black' : 'text-[var(--text)]/40'} p-4 rounded-xl`}><Monitor className="w-8 h-8" /></button>
                <button onClick={() => setDevice('tablet')} className={`${device === 'tablet' ? 'bg-white text-black' : 'text-[var(--text)]/40'} p-4 rounded-xl`}><Tablet className="w-8 h-8" /></button>
                <button onClick={() => setDevice('mobile')} className={`${device === 'mobile' ? 'bg-white text-black' : 'text-[var(--text)]/40'} p-4 rounded-xl`}><Smartphone className="w-8 h-8" /></button>
              </div>
          ...
          <div className="flex-1 overflow-auto p-20 bg-gradient-to-br from-black via-purple-950/10 to-black">
            <Reorder.Group axis="y" values={blocks} onReorder={setBlocks} className="max-w-7xl mx-auto">
              {blocks.length === 0 ? (
                <div className="h-screen flex flex-col items-center justify-center text-[var(--text)]/20">
        ...
        <AnimatePresence>
          {selectedId && selected && (
            <motion.div initial={{ x: 400 }} animate={{ x: 0 }} exit={{ x: 400 }} className="w-96 border-l border-white/5 bg-[var(--bg)]/90 p-12">
              ...
              {Object.entries(selected.content).map(([key, value]) => (
                <div key={key} className="mb-10">
                  ...
                  <textarea
                    ...
                    className="w-full mt-4 px-8 py-6 bg-white/5 border border-[var(--border)] rounded-2xl text-2xl text-[var(--text)] focus:border-purple-500/50 outline-none min-h-[120px]"
        ...
        <motion.div className="fixed inset-x-12 bottom-20 z-50">
          <div className="max-w-7xl mx-auto relative">
            <input
              ...
              className="w-full px-24 py-16 text-5xl font-light bg-[var(--bg)]/80 backdrop-blur-3xl border-8 border-purple-500/70 rounded-3xl outline-none text-[var(--text)] placeholder-white/30 shadow-2xl"
            />
