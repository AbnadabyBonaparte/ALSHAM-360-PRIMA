...
const NumberTicker = ({ value, prefix = "" }: { value: number | string, prefix?: string }) => (
  <span className="tabular-nums tracking-tight">{prefix}{value.toLocaleString('pt-BR')}</span>
);

const TimeTravelDial = ({ mode, onChange }: { mode: 'past' | 'present' | 'future', onChange: (m: 'past' | 'present' | 'future') => void }) => (
  <div className="flex items-center gap-1 rounded-full border border-[var(--border)]/50 bg-[var(--surface-strong)]/50 p-1 backdrop-blur-md" role="radiogroup" aria-label="Seletor Temporal">
    {[
      ...
      <button
        ...
        role="radio"
        className={`relative rounded-full px-3 sm:px-4 py-1.5 text-[0.625rem] sm:text-xs font-medium uppercase tracking-wider transition-all ${mode === m.id ? 'text-white' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
      >
        {mode === m.id && (
...
const OracleWidget = ({ kpis, mode }: { kpis: any[], mode: string }) => {
  const getInsight = () => {
    ...
    if (leads === 0) return { title: "Alerta de Topo de Funil", impact: "Zero leads detectados. Prioridade crítica: ativar campanhas.", confidence: "Crítica" };
    if (revenue > leads * 10000) return { title: "Anomalia de Conversão", impact: "Mais receita que leads? Verifique dados.", confidence: "Média" };
      ...
      <header className="flex flex-col gap-4 sm:gap-6 md:flex-row md:items-center md:justify-between">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <span className={`rounded-full border px-2 sm:px-3 py-0.5 sm:py-1 text-[0.625rem] sm:text-xs font-bold uppercase tracking-widest ${timeMode === 'future' ? 'border-[var(--accent-fuchsia)]/30 bg-[var(--accent-fuchsia)]/10 text-[var(--accent-fuchsia)]' : 'border-[var(--accent-emerald)]/30 bg-[var(--accent-emerald)]/10 text-[var(--accent-emerald)]'}`}>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          <button
            ...
            className={`group relative flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full border transition-all ${isListening ? 'border-[var(--accent-alert)] bg-[var(--accent-alert)]/10 text-[var(--accent-alert)] animate-pulse' : 'border-[var(--border)] hover:border-[var(--accent-sky)] text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
